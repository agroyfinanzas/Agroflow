...
function exportarLotesParaSincronizacion() {
  ...
  // ✨ Sincronización DIRECTA con el iframe del Planificador.
  // No dependemos solo del evento 'storage' del navegador (que a veces
  // no se dispara de forma confiable entre iframes según el navegador).
  // Si el iframe ya está cargado y expone sus funciones, las llamamos
  // directamente para forzar el refresco inmediato.
  try {
    const iframe = document.getElementById('planificador-iframe');
    if (iframe && iframe.contentWindow) {
      const win = iframe.contentWindow;
      if (typeof win.sincronizarLotes === 'function') win.sincronizarLotes();
      if (typeof win.consolidarInsumos === 'function') win.consolidarInsumos();
      if (typeof win.actualizarResumenLotes === 'function') win.actualizarResumenLotes();
    }
  } catch (e) {
    // Si el iframe todavía no cargó o hay una restricción del navegador,
    // no rompemos el flujo principal; el evento 'storage' actúa como respaldo.
    console.warn('⚠️ Sincronización directa con iframe no disponible aún:', e.message);
  }
